...
const FEEDS: { url: string; source: string; category: string; types: string[] }[] = [
  ...
  {
    url: 'https://www.hankyung.com/feed/global-market',
    source: '한국경제',
    category: '해외증시',
    types: ['stock_us', 'etf_us'],
  },
  {
    url: 'https://www.yna.co.kr/rss/economy.xml',
    source: '연합뉴스',
    category: '경제',
    types: ['savings', 'real_estate', 'insurance', 'cma', 'precious_metal'],
  },
  {
...
// in-memory cache: key = sorted types string
...
export async function GET(request: Request) {
  ...
  // Select feeds relevant to the user's asset types; fall back to all if none specified
  const selectedFeeds = activeTypes.length > 0
    ? FEEDS.filter((f) => f.types.some((t) => activeTypes.includes(t)))
    : FEEDS

  // Deduplicate selected feeds by URL
  const uniqueFeeds = selectedFeeds.filter((f, i, arr) => arr.findIndex((x) => x.url === f.url) === i)

  const cacheKey = uniqueFeeds.map((f) => f.url).sort().join('|')
  const now = Date.now()
  const hit = cache.get(cacheKey)
  if (hit && now - hit.at < CACHE_TTL) {
    return NextResponse.json(hit.items)
  }

  const results = await Promise.all(uniqueFeeds.map(fetchFeed))

  // Interleave items from different feeds so categories are mixed, max 6
  ...
  if (merged.length > 0) {
    cache.set(cacheKey, { items: merged, at: now })
  }

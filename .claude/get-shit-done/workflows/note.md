...
- **Global scope**: `/Users/john/dev/jackpot-pb/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/john/dev/jackpot-pb/.claude/notes/`)
...
2. Glob `/Users/john/dev/jackpot-pb/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/john/dev/jackpot-pb/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/john/dev/jackpot-pb/.claude/notes/` — works in any directory

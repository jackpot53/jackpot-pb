...
Plans:
- [x] 05-01-PLAN.md — Goal CRUD: Server Actions + DB queries + dialog components (GoalDialog, DeleteGoalDialog) + /goals page shell + dashboard goals section with Progress bars
- [x] 05-02-PLAN.md — Goal progress chart: GoalProgressChart AreaChart + ReferenceLine per goal + /goals page integration with snapshot data
- [x] 05-03-PLAN.md — Performance comparison view: /performance page + PerformanceFilterClient with shadcn Tabs (전체/주식/코인/예적금/부동산) + sidebar 성과 nav item
